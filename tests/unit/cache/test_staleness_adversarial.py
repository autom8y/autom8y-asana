"""Adversarial tests for staleness detection.

Per VP-CACHE-LIGHTWEIGHT-STALENESS: Validates edge cases, race conditions,
boundary conditions, and failure modes that are difficult to trigger in
normal unit tests.

Test Categories:
- Race conditions in coalescer (100+ concurrent requests)
- Timer edge cases (request at 50ms boundary)
- Batch overflow (200+ requests in rapid succession)
- API timeout during batch
- Malformed modified_at handling
- Entry deleted during check (404 response)
- TTL ceiling boundary verification
- Extension count overflow (massive values)
"""
