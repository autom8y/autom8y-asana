...
class TestGidValidation:
    """Tests for GID format validation at track time."""

    def test_empty_gid_raises_validation_error(self) -> None:
        """Empty string GID raises ValidationError with actionable message."""
        tracker = ChangeTracker()
        task = Task(gid="", name="Test Task")

        with pytest.raises(ValidationError) as exc_info:
            tracker.track(task)

        assert "empty string" in str(exc_info.value).lower()
        assert "Use None for new entities" in str(exc_info.value)

    def test_malformed_gid_raises_validation_error(self) -> None:
        """Malformed GID (letters) raises ValidationError with actionable message."""
        tracker = ChangeTracker()
        task = Task(gid="abc123", name="Test Task")
    ...
    def test_special_chars_gid_raises_validation_error(self) -> None:
        """GID with special characters raises ValidationError."""
        tracker = ChangeTracker()
        task = Task(gid="123-456", name="Test Task")
    ...
    def test_valid_numeric_gid_accepted(self) -> None:
        """Valid numeric GID is accepted without error."""
        tracker = ChangeTracker()
    ...
    def test_valid_temp_gid_accepted(self) -> None:
        """Valid temp_* GID is accepted without error."""
        tracker = ChangeTracker()
        task = Task(gid="temp_12345", name="Test Task")
    ...
    def test_none_gid_accepted_by_validation_method(self) -> None:
        """None GID is accepted by validation method (defensive for future models).

        Note: Current AsanaResource model requires GID as str, so Task(gid=None)
        raises Pydantic validation. This test verifies the tracker's internal
        validation method handles None defensively.
        """
        tracker = ChangeTracker()

        # Test the validation method directly - it should accept None
        # for future models that may have optional GIDs
        tracker._validate_gid_format(None)  # Should not raise

    def test_gid_with_spaces_raises_validation_error(self) -> None:
        """GID with spaces raises ValidationError."""
        tracker = ChangeTracker()
        task = Task(gid="123 456", name="Test Task")
    ...
    def test_gid_with_leading_zeros_accepted(self) -> None:
        """GID with leading zeros is valid (numeric string)."""
        tracker = ChangeTracker()
        task = Task(gid="00123456", name="Test Task")

        # Should not raise - leading zeros are valid in numeric strings
        tracker.track(task)

        state = tracker.get_state(task)
        assert state is not None

    def test_temp_gid_without_number_raises_validation_error(self) -> None:
        """temp_ prefix without number raises ValidationError."""
        tracker = ChangeTracker()
        task = Task(gid="temp_", name="Test Task")
    ...
    def test_temp_gid_with_letters_raises_validation_error(self) -> None:
        """temp_ prefix with letters instead of number raises ValidationError."""
        tracker = ChangeTracker()
        task = Task(gid="temp_abc", name="Test Task")
...
class TestSessionValidation:
    ...
    def test_session_track_validates_gid(self) -> None:
        """Session.track() validates GID format."""
        mock_client = create_mock_client()

        with SaveSession(mock_client) as session:
            task = Task(gid="invalid-gid", name="Test")
