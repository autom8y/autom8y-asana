...
class TestWebhookDispatcherConfig:
    ...
    def test_from_env_case_insensitive_booleans(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("WEBHOOK_DISPATCH_ENABLED", "TRUE")
        monkeypatch.setenv("WEBHOOK_DISPATCH_DRY_RUN", "FALSE")
...
class TestLifecycleWebhookDispatcher:
    ...
    def _make_dispatcher(
        ...
        *,
        ...
        config = WebhookDispatcherConfig(
            ...
            allowed_event_types=(
                event_types if event_types is not None else frozenset({"section_changed"})
            ),
        )
        mock_dispatch = AsyncMock()
        mock_dispatch.dispatch_async = AsyncMock(return_value=dispatch_result or {"success": True})
    ...
    def test_disabled_short_circuits(self) -> None:
        ...
        result = self._run(dispatcher.handle_event("section_changed", "Process", "gid1", {}))
    ...
    def test_entity_type_not_allowed(self) -> None:
        ...
        dispatcher, mock_dispatch, _ = self._make_dispatcher(entity_types=frozenset({"Process"}))
        result = self._run(dispatcher.handle_event("section_changed", "Offer", "gid1", {}))
        assert result["dispatched"] is False
    ...
    def test_live_dispatch_success(self) -> None:
        ...
        dispatcher, mock_dispatch, _ = self._make_dispatcher(dispatch_result={"success": True})
