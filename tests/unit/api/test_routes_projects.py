...
# NOTE: GET /api/v1/projects/{gid}/sections was hardened into a scoped,
# token-safe S2S read capability (FORK-R REUSE branch). Its contract changed
# (JWT-only auth + single-project allowlist), so the former dual-mode PAT tests
# were moved and rewritten as hard-gate tests in
# ``tests/unit/api/routes/test_projects_sections_hardened.py`` (H1–H6).
