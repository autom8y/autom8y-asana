...
| **E-6 — PROVISIONAL, proxy-measured 2026-09-16T17:21Z** | **The founding office's arrivals may not all be its own.** The EBI locus describes the incoming park rule as *"for the sink office's forwarded flood — one office relaying twenty-eight senders"*, keyed on office + envelope-path; the client-impact seat describes the same rule as *"for the founding office's non-booking mail (`ccb52f4c`)"*. **Whether the sink office IS `ccb52f4c` is asked, not inferred.** Proxy from this plane: over 72 h, `ccb52f4c`'s **130 arrival traces join to 25 distinct From domains, one carrying 58.5 % and three carrying 79 %** (parsed header From, masked; every capped query printed complete). Twenty-five beside twenty-eight, from a different key, is the same shape. **If confirmed, the RATE floor has been reading an office whose denominator is mostly relayed mail — 21 of 40 runs on the floor at 1–4 % may be substantially a plumbing shape, not the clinic's booking behaviour.** The floor is not wrong (a relayed lead that never books is still a non-booking); what changes is what the reader should *do* when this office appears, which the brief currently states as "it needs action". | `arm_observe.py C ccb52f4c` prints the top From domain and its share; the confirming read is the EBI lane's envelope-path key, which this seat cannot reach | 25 senders, top-1 58.5 %, top-3 79.2 %, joined 130 of 130. **Withdrawn with equal visibility if the sink is a different office; the share and the key go in beside this number if it is the same.** |
