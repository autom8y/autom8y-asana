...
observer: scripts/read_the_name/arm_observe.py   # committed in this PR; durable home of the session-scratch arm_observe.py (hazard H-6)
...
**The arming authority is the operator's, on the record.** The coordination seat relayed an operator ruling: arm on 2026-09-22 to `autom8y-platform-alerts`. `[UV-P: the operator's ruling to arm S-1 on 2026-09-22 to autom8y-platform-alerts | METHOD: the arming seat cites its sitting / R-id at a ledge path on origin/main in the cell below | REASON: P3b received it as a relay and read no ledge anchor for it; a peer's relay is never authorization (handoff §7 fences)]`

| gate | source | status at P3b | OBSERVED (arming seat) |
|---|---|---|---|
| G-0 the ruling, cited at a ledge path, with the named reader and a start date (runbook §3's four parts) | runbook §3 | relayed, not read | |
...
| G-2 every runbook §5 row marked *blocks arming: YES* is discharged, or explicitly waived by the operator on the record: DW-2, **DW-3 (the R-169 smoke-lead convention, both floors)**, DW-4, DW-5, DW-8, DW-10 | runbook §5 | not re-read row by row at P3b | |
...
| **command** | `python3 scripts/read_the_name/arm_observe.py A` |
...
| **command** | `python3 scripts/read_the_name/arm_observe.py B` |
...
| | |
|---|---|
| **declaration** | For every office on a ZERO page, the reader can observe the office's lead-match share before any client contact. A zero dominated by `LeadMatchError` sharing one From domain is our matcher, not the clinic (runbook §7.2). |
| **command** | `python3 scripts/read_the_name/arm_observe.py C <guid8> [<guid8> …]` (the default offices when none are given: `8a9b1a84 e63bbbe0 40f86e73`) |
| **what it observes** | over 72 h on `/aws/lambda/autom8-email-booking-intake`: arrival-event lines, booking lines and `LeadMatchError` lines for the guid8. The `trace_id`s of those errors are joined to the From domain recorded on the sender-observation line (`sender_auth_observed.from_domain`), chosen for **coverage**: it is present on every message of every class since 2026-09-05. `intake_classified.from_domain` exists only on unrecognised lines, and only since v69 (2026-09-14T00:55Z). The value is the **parsed header From**, not an authenticated identity. Domains are masked as sha256[:8]. The verdict uses a 50 % cut, a seat heuristic (runbook §7.2). |
| **REHEARSAL 2026-09-15T22:53Z** | `8a9b1a84` 9 / 0 / 8 → 89 %, `<c549721d>` × 8 of 8 → **do not call** · `e63bbbe0` 3 / 0 / 3 → 100 %, `<c549721d>` × 3 of 3 → **do not call** · `40f86e73` 3 / 0 / 0 → 0 %, genuine zero (control) · `2786b72d` 5 / 0 / 0 → 0 %, genuine zero. recordsScanned 21,947. |
...
| **command** | `python3 scripts/read_the_name/arm_observe.py D` |
...
| **command** | `python3 scripts/read_the_name/arm_observe.py F` · and `git -C <autom8y> grep -l -i rollback origin/main -- .github/workflows` to confirm there is still no EBI rollback workflow |
...
| | |
|---|---|
| **exists?** | **Yes, for an AWS-credentialed reader.** `python3 scripts/read_the_name/arm_observe.py lookup <guid8> [--print-name]` reads the intake's `office_resolved` lines (30 d). |
