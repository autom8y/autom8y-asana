...
**E-3 (D1 — the de-dup factor).** `message_id` de-duplicates nothing (`count_distinct == count` on `terminal_decline`, the only event carrying it), so U-3 reduces to counting terminal-outcome lines with parks de-duplicated by `park_key` (99.6 % co-occurrence, all asymmetry decline-without-park). The ×1.30 headroom at r = 0.025 was computed with bookings pinned; in window C the factor reached 2.65 (ceiling 2.15) and the founding office fired the RATE floor because its bookings fell to 1. **Recorded as a calibration hazard, not re-tuned** (K1–K5 pass on rite-disjoint re-derivation).
