...
## ERRATUM — 2026-09-14, after the Phase-B critiques (change-warden on S1.2, qa-adversary on S1.1); part of this ADR, travels with it

**E-1 (D2.4 and D2.7 — page topic).** Both sections still rule the deadman instances' `alarm_actions` at `platform_alerts` and reject a scratch topic. **Sitting XI R-168 governs and displaces that text:** with no consumer word, `platform_alerts` *is* the void, and every page path — the digest publish (Lambda env) and every alarm's actions (prober; the success-gap alarm when created at S1.7) — reads ONE variable `office_floor_page_topic_arn` whose value is the stack's scratch topic `autom8-ebi-office-floor-scratch`. The soak-then-create sequencing of D2.4 survives; the two-instance topic-split rationale does not. The re-point on the operator's word is that one variable, and its receipt must prove BOTH consumers (an alarm-driven SNS MessageId AND a digest-run MessageId on `platform_alerts`), two-sided against the scratch topic.

**E-2 (D5 — the class is context, never a suppressor).** Joined to the landed snapshot (asana #449, 29 rows), both true stalls in the S1.1 K3 set (`87bd31d7`, `6b93fb76`) carry class `inactive` at the Offer grain while the sizing read shows them booking 15 and 10 times. A digest that routed "inactive" to EXPECTED SILENCE would reproduce the founding silence inside the instrument built to end it. **Amendment:** the evaluator carries, per office, `last_booking_age_days` (a 30-day plane-derived lookback from the same widened query as the day-N counter; null if none). Sectioning: over a floor AND a booking within 30 days → **ACTIONABLE** regardless of class (a stall); over a floor, no booking in 30 days, class ∈ {inactive, ignored} → **EXPECTED SILENCE**; over a floor, no booking in 30 days, class unknown → **CLASS UNKNOWN**. The snapshot's vocabulary is six-valued on the page: `active · activating · inactive · ignored · unknown` and the `***` residual. E4 (plane-derived booking history) is thereby adopted as a **companion** to E2, not a replacement. **This amends D8.1's section predicate explicitly:** `[1] ACTIONABLE` is no longer keyed on `class ∈ {active, activating}`; it is keyed on `last_booking_age_days ≤ 30` (a stall) OR `class ∈ {active, activating}` with no booking in 30 days (a live client that stopped), and the builder's guard test asserts a class-inactive office with a 12-day-old booking lands in ACTIONABLE (runbook DW-10).

**E-3 (D1 — the de-dup factor).** `message_id` de-duplicates nothing (`count_distinct == count` on `terminal_decline`, the only event carrying it), so U-3 reduces to counting terminal-outcome lines with parks de-duplicated by `park_key` (99.6 % co-occurrence, all asymmetry decline-without-park). The ×1.30 headroom at r = 0.025 was computed with bookings pinned; in window C the factor reached 2.65 (ceiling 2.15) and the founding office fired the RATE floor at 2.02 % because its bookings fell to 2 (the seat's pre-sitting count of 1 was taken before window C closed; S1.1 and its critic both measure 2). **Recorded as a calibration hazard, not re-tuned** (K1–K5 pass on rite-disjoint re-derivation).

**E-4 (D1 — K2).** K2 is a tautology given the floors (rate ≥ 7 % ⇒ bookings ≥ 1 ⇒ ZERO excluded; 7 % > 2.5 % ⇒ RATE excluded); it is a consistency check, not an independent gate.
