...
| **11 · NEW · HARD STOP** | `service-terraform.yml`'s manual apply (its own header: "the ONLY apply path") runs `terraform-apply-reusable.yml`, which passes **no `image_tag`**. `production.tfvars:344` pins `image_tag = "67d89d7"` (2026-09-05); only `service-deploy-lambda.yml` overrides it (`-var` AFTER `-var-file`, :261/:274, last assignment wins). **A terraform-only apply of this stack rolls all four Lambdas back to the 09-05 image** (#2073 · #2105 · #2125 · #2149 · #2166 · #2169 undone in one stroke). Per name-the-wave (A78): both #2165's and #2170's plans carried `~ image_uri 3aa5c94 → 67d89d7`; #2165 was safe only because #2169's code deploy applied it instead. Pre-existing, comment-narrated, verified own-hands at `bc9621a4` and live (alias `live` → v66, image `3aa5c94`, no dispatch run since 20:08Z 09-10). | **Open. No TF-only apply for EBI from any seat.** Word path for TF-only changes = `service-deploy-dispatch` `workflow_dispatch` (`service_name=email-booking-intake`), or the next `services/email-booking-intake/**` merge, unattended. **#2170 is on main with the HealthCom window set: the next such merge by anyone turns body retention on with no word.** Check `ads` and `pull-payments` for the same pin pattern before re-pinning the freshness module (§3 `VALIDATE-null-guard`). |
| **12 · NEW · PII** | `office_phone` lands in CloudWatch in plaintext: `stage_exception` lines with `error_type: SlotConflictError` embed the scheduling API's error payload verbatim, and that payload carries `office_phone`. Found by pythia (CONSULT-morning-after-north-2026-09-11 S-5, "observed twice in one sample"); **this seat reproduced it by Logs Insights on `/aws/lambda/autom8-email-booking-intake` since 09-01: 119 SlotConflictError lines, 119 carry `office_phone`, 119 are `stage_exception` — every one, not a sample.** The witness ledger DROPS `office_phone` by ruling (`ebi_witness_ledger.py`); the exception path never got the ruling. A first `filter-log-events` read returned 0 for the same term (single 1MB page): the untaken-zero class, again. | **Open. NO WATCHER.** Cure is an EBI code change (strip/redact the embedded payload on `stage_exception`), in-grant under R-120, **deliberately not built this morning**: any `services/email-booking-intake/**` merge auto-applies #2170 (HealthCom window) and #2171 unattended, and the go-live choice for #2170 is the operator's (row 11). Build after that word. |
...
| **VALIDATE-null-guard** | `lambda-freshness-deadman` **v1.4.0** validation rules (`var.x == null \|\| (...)`) fail `terraform validate` under Terraform 1.11.x (this stack: `required_version >= 1.11`). Filed by name-the-wave 2026-09-11, proven by control with their files reverted; not caused or fixed by #2165. Fixed upstream at a8 **v1.4.1** (`== null ? true : (...)`, all four rules), which ALSO drops the success-gap soft gate (`&& length(alarm_actions) > 0`) for a HARD plan-time refusal. Six pins: 5× v1.4.0 (ads · asana · email-booking-intake · financial-tier-bot-alarms ×2), 1× v1.3.18 (pull-payments). Only ads arms the success gap, with a channel, so a v1.4.0→v1.4.1 re-pin is plausibly resource-inert for the five and UNPROVEN by plan; three consumer stacks (ads · EBI · pull-payments) auto-apply any re-pin on their next `services/**` merge (carrier #2). | operator word: re-pin after a per-stack plan | **NO WATCHER** |
...
6. **2026-09-11 morning, two rows of sitting VIII falsified at the live plane — see `ERRATUM-sitting-VIII-apply-status-and-retain-lever-2026-09-11.md` (#435).** E-1: #2165 was APPLIED unattended at 20:15:50Z by #2169's `service-deploy-dispatch` run (the "one apply is the operator's word" row is moot; carrier #2 demonstrated). E-2: the HealthCom body-retain lever never reached traffic (alias `live` → v66, 45 vars, no retain var; the hand-set var is on `$LATEST` only), so RATIF-VIII-A4-EDGE has **no effective watcher** until name-the-wave's #2170 is applied. E-3: this seat's own wrong-object read, twice: `get-function-configuration` with no `--qualifier` attests `$LATEST`. Fence banked.
