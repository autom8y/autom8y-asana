...
Vocabulary: `IN SOAK · day 0` = BLOCKED(on the ≥7-day clock, nobody's word); `LANDED-BEFORE-WAVE` = DONE (verify-not-build); `THIS DOCUMENT` = DONE at its merge. Gate C flagged the three as outside the enum; they are kept because each is more exact than its enum member.
...
| T4 `/handoff` | THIS DOCUMENT | Gate C FLAG-ADVISORY (six flags F-1..F-6, none falsifying; line one honest; receipt (ii) reproduced to the digit; every flag cured in the follow-up commit on this PR) | verification-auditor |
...
- **(i) PROBE LIVE** — **PROVEN LIVE, two-sided** (S1.4b, asana #456 → `49b6fe29`; own-hands corroboration 06:41Z: `aws logs filter-log-events --log-group-name /aws/lambda/autom8-email-booking-intake-office-floor --start-time <05:50Z ms> --filter-pattern '"office_floor_evaluated"' --output json` rc 0, 4 events, one page; `aws cloudwatch get-metric-statistics --namespace Autom8y/EbiOfficeFloor --metric-name LastSuccessTimestamp --start-time 2026-09-14T05:50:00Z --end-time <now> --period 60 --statistics SampleCount Maximum` rc 0, 2 datapoints; `aws cloudwatch get-metric-statistics --namespace AWS/SNS --metric-name NumberOfMessagesPublished --dimensions Name=TopicName,Value=autom8-ebi-office-floor-scratch --start-time 2026-09-14T05:50:00Z --end-time <now> --period 300 --statistics Sum` rc 0, 2+1+1; `aws sns list-subscriptions-by-topic --topic-arn arn:aws:sns:us-east-1:<ACCOUNT>:autom8-ebi-office-floor-scratch --query 'length(Subscriptions)'` rc 0 → 0): the positive pole — a healthy invoke and the scheduled fire both evaluate 39 offices with the control PASSED and emit the success timestamp; the negative pole — a 10-minute window is REFUSED (`records_scanned_below_floor`) with the timestamp withheld, and a dry-run evaluates without emitting; page paths: zero evaluator publishes (hour ≠ 11) and the four topic publishes are alarm OK-transitions to a topic with 0 subscribers. The SNS delivery of a real digest (`sns:Publish`) stays UNPROVEN-LIVE until the 11:27Z evaluation
...
| WS-2 build charge + dated consumer commitment | operator (next sitting) | the next decision-space sitting (sitting XII) | NO WATCHER |
| WS-3 (c2) coverage behind the identity one-pager (R-110) | operator | the R-110 identity one-pager sitting | NO WATCHER |
| WS-4 GHL contactId un-scaffold | operator | after WS-1..3 produce its control | NO WATCHER |
| R-143 charges (apply-visibility notice · Service Terraform rollback guard); asana pin drop on the #2200 precedent | to be seated | the operator's word seating the R-143 charge (asana pin drop rides the #2200 precedent) | NO WATCHER |
| Calendly drafts #1561/#1563/#1564 + recon rule (R-153 untouched); orphaned tripwire IaC routing | operator (monolith owner nominated) | the operator's word to the monolith owner (nothing sent) | NO WATCHER |
| S-4 sitting items (the tripwire cannot separate S-4 from the other `***` populations — re-ask against 94 %, not 11 % · the `e5a68603` phantom KEY (`guid8_of()` has no validation; not a snapshot row; cannot reach A=5 today at 3 lines) in D5.4 · split `absent` · arming any of the six unwatched metrics); the envelope-fallback local-part cure is a seat's once ruled | operator (sitting XII); cure seat after the ruling | sitting XII | NO WATCHER (measured: 6 filters, 0 alarms) |
| `15caa02c` class — RULED (`RULING-read-the-name-t2-15caa02c-class-2026-09-14.md`); the two halves pythia named: (a) render `⚠ DARK-BUT-BOOKING` on the join E-2 already performs; (b) provenance to the WS-2 offer-grain observer | (a) S1.3/S1.4 builder seat; (b) WS-2 build seat | (a) next evaluator change; (b) a sourced refresh inserting `15caa02c: inactive` while its last `section_changed` is an active-class move | NO WATCHER (SIZING-15caa02c's "NO WATCHER" is superseded only once (a) renders on a page someone reads) |
| re-examine the non-required status of `Guard teeth tests (hermetic)` (change-warden P-1) and the split concurrency groups (P-2) | operator (next sitting) / R-143 charge | sitting XII; the R-143 charge's seating | NO WATCHER |
...
| `[UV-P: office_name is on booking lines before 2026-09-09 | METHOD: key inventory on pre-09-09 booking lines | REASON: T2's inventory came from 25 lines inside the stamped era]` | WS-1 successor | the next read that cites a pre-09-09 booking figure | NO WATCHER |
...
| S1.4b F-3: two ZERO-floor offices (`e63bbbe0` 9 arrivals, `8a9b1a84` 7) sit in CLASS UNKNOWN on the first live run and the run line carries no `offices_unclassified` (C-4 coverage counter) | S1.3 builder seat, before the arming word | next evaluator change | NO WATCHER (the digest prints it daily to a topic with 0 subscribers — that is not a watcher) |
...
| F7 ceiling (Insights scan cost if the plane grows ×10) | seat | a 3-day scan > 50 MB (read on every soak row: `records_scanned`) | the soak table |
| stale worktrees of earlier seats under autom8y `.knossos/worktrees/` (8 listed, none this wave's) | hygiene | the next `/naxos` pass | NO WATCHER |
