...
- **Q1 — UV-P-5:** WHICH Cloudflare account owns Pages project `deck-host` and
  `decks.cntently.com`? Candidate `a245df42…[REDACTED-ACCOUNT-ID]` is a **file-read
...
| **UV-P-5** (account ownership of `deck-host` + `decks.cntently.com`) | **OPEN — the operator's FIRST question.** Candidate `a245df42…[REDACTED-ACCOUNT-ID]` is a **file-read CLAIM, unverified**; the credential on this machine lists only `tenuta-decks`; the DNS zone was never probed | rides under Gate C | DP-2 `:36-38`, `:1659` |
