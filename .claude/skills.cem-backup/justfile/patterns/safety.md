...
### CI-Aware Confirmation

The confirmation helper automatically detects CI and skips the prompt:

```just
# In CI: auto-proceeds
# Locally: prompts user
db:seed: (_confirm "Seed database with test data?")
    ./scripts/seed-db.sh
```

**Alternatives for CI behavior**:

```just
# Option 1: Auto-confirm in CI (shown above)
# Option 2: Block in CI (for truly dangerous ops)
[private]
...
Usage:
```just
deploy: (_require_env "AWS_PROFILE") (_require_env "AWS_REGION")
    aws deploy ...
```
