...
_log msg:
    @echo "==> {{msg}}"
```
- Use `@` prefix to suppress recipe echo
- Consistent prefix (`==>`) for visual scanning
...
_require_env var:
    @[ -n "${{{var}}:-}" ] || \
        (echo "Required environment variable not set: {{var}}" >&2 && exit 1)
```
- Uses `${VAR:-}` pattern for safe empty check
- Triple braces: `{{{{var}}}}` becomes `${VAR:-}`

**Confirmation with CI detection**:
```just
[private]
_confirm msg:
    #!/usr/bin/env bash
...
```
- Shebang for multi-line script
- CI auto-confirms (or auto-skips - choose per project)
...
**Bootstrap with tool check**:
```just
bootstrap: _check_tools
    {{UV}} sync
```
- Validates tools before running
...
clean:
    find . -type d -name __pycache__ -exec rm -rf {} + 2>/dev/null || true
```
- `|| true` prevents failure if nothing found
