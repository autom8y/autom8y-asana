...
| Setting | Purpose |
|---------|---------|
| `dotenv-load := true` | Load `.env` file automatically |
| `export := true` | Export all variables to recipe environment |
| `shell := ["bash", "-eu", "-o", "pipefail", "-c"]` | Safe shell defaults (exit on error, undefined var, pipeline failure) |
