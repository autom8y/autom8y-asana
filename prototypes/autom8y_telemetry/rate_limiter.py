"""Token bucket rate limiter extracted from autom8_asana.

This is prototype code - extracted and decoupled from autom8_asana.
Production version would live in autom8y_telemetry package.

SHORTCUTS TAKEN (see README.md):
- Uses generic RuntimeError instead of ConfigurationError
- Removed autom8_asana dependency on exceptions module
- Logger is optional and uses Protocol instead of concrete type
"""
...
if TYPE_CHECKING:
    from .protocols import LogProviderProtocol
...
class TokenBucketRateLimiter:
    ...
    Implements RateLimiterProtocol.
    """
    ...
    def __init__(
        ...
        logger: LogProviderProtocol | None = None,
    ) -> None:
        ...
        Raises:
            RuntimeError: If max_tokens or refill_period are invalid
        """
        if max_tokens <= 0:
