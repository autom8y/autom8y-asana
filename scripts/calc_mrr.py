#!/usr/bin/env python3
"""Calculate MRR from Business Offers using section classification.
...
MRR is a Unit-level value that cascades to sibling Offers. To avoid
inflation, results are deduped by (office_phone, vertical) -- the
phone-vertical pair that uniquely identifies a Unit.

Usage:
    python scripts/calc_mrr.py
    python scripts/calc_mrr.py --verbose
    python scripts/calc_mrr.py --classification activating
...
# OFFER_CLASSIFIER active group -- mirrors activity.py exactly.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calculate MRR using section classification groups",
    )
    parser.add_argument(
        "--classification", "-c",
        default="active",
    ...
    # Filter to rows with valid MRR
    with_mrr = classified.filter(pl.col("mrr").is_not_null()).with_columns(
        pl.col("mrr").cast(pl.Float64).alias("mrr_f")
    ).filter(pl.col("mrr_f") > 0)

    if len(with_mrr) == 0:
        print("\nNo offers with MRR > 0 found.")
        return

    print(f"\nOffers with MRR > 0: {len(with_mrr)}")
    print(f"Raw sum (before dedup): ${with_mrr['mrr_f'].sum():,.0f}")

    # Dedup by (office_phone, vertical) -- take first MRR per combo
    deduped = with_mrr.group_by(DEDUP_KEYS).agg(
        pl.col("mrr_f").first().alias("mrr"),
        pl.col("office").first().alias("office"),
        pl.len().alias("offer_count"),
    ).sort("mrr", descending=True)

    total_mrr = deduped["mrr"].sum()
    print(f"Unique ({', '.join(DEDUP_KEYS)}) combos: {len(deduped)}")
    print(f"\n  Active MRR: ${total_mrr:,.0f}\n")

    if args.verbose:
        print("Per-unit breakdown:")
        print(f"  {'Phone':<16s} {'Vertical':<20s} {'Office':<30s} {'MRR':>10s} {'Offers':>6s}")
        ...
        for row in deduped.iter_rows(named=True):
            ...
            print(f"  {phone:<16s} {vert:<20s} {office:<30s} ${row['mrr']:>9,.0f} {row['offer_count']:>6}")
        print(f"  {' ' * 68}----------")
        print(f"  {'TOTAL':<68s}${total_mrr:>9,.0f}")
