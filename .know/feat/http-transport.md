...
- `ADR-0048`: Circuit breaker opt-in pattern — `CircuitBreakerConfig.enabled=False` by default (TRADE-002). The circuit breaker is available but disabled for backward compatibility during build-out.
- `ADR-0009`: `get_stream_url` is used for streaming attachment download from external URLs (not the Asana API base URL).
- **IMP-10** (`a114c619`): `ConfigTranslator.to_http_client_config` intentionally passes only `timeout.connect` (not `timeout.read`) as the platform SDK timeout parameter for faster failure detection under degraded network conditions.
...
**Rate plane (sustained throughput):** A `TokenBucketRateLimiter` limits the request rate to Asana's documented quota (default 1500 req/60 sec from settings, or 1000 in `AsanaConfig` test factory). Tokens are consumed before each request. The bucket drains at the configured `max_requests / window_seconds` rate. When the bucket is empty, requests block until tokens refill. This plane is time-based.
...
**Lazy client initialization** with double-checked locking (`_get_client()`): `_platform_client` starts as `None`. The first `await self._get_client()` acquires `_client_lock`, creates the `Autom8yHttpClient`, and directly mutates its internal `_client.headers` to inject the Authorization Bearer token. Subsequent calls take the fast path (no lock). Token strings are ASCII-sanitized before injection (`str(token).encode("ascii", errors="replace").decode("ascii")`) to prevent `UnicodeEncodeError` from fuzzed token values (hardened in `6b93faf4`). This defers token acquisition until the first request.
...
**AIMD Phase 1 status (TRADE-003):** The `cooldown_duration_seconds` field in `AIMDConfig` is a placeholder. The cooldown warning fires at `cooldown_trigger` (default 5) consecutive 429s but does not reduce throughput further. The log note `"cooldown_not_active_in_v1"` is present at `transport/adaptive_semaphore.py:339`. Operators seeing this warning have no further automatic protection beyond the AIMD window halving.
...
`AsanaHttpClient._request(method, path, ...)` — inner loop for all non-streaming, non-multipart requests. All retry and AIMD feedback happens here. Selects `_read_semaphore` for GET, `_write_semaphore` for all other methods.

`AsanaHttpClient._request_paginated(...)` — identical structure to `_request` but calls `response_handler.unwrap_paginated_response()` and always uses `_read_semaphore`. Note: the loop body is structurally duplicated from `_request` — deliberate to avoid branching in the hot path (known code smell).

`AsanaHttpClient.post_multipart(...)` — uses the `raw()` escape hatch from `Autom8yHttpClient` to send `multipart/form-data`. Temporarily removes the `Content-Type` header so httpx auto-generates the boundary. `finally` block restores the header unconditionally.

`AsanaHttpClient.stream(...)` — `@asynccontextmanager` yielding raw `httpx.Response` for streaming downloads. Applies rate limiting once per initiation; no AIMD feedback (streaming duration is unbounded). No concurrency semaphore consumed.

`AsanaHttpClient.get_stream_url(url)` — async generator (not context manager) yielding `bytes` chunks from an external URL. Per ADR-0009. Rate limiting applied; no semaphore acquired; not protected by retry/circuit breaker.

`AsyncAdaptiveSemaphore._handle_reject(slot_epoch)` — synchronous (no await points); epoch check then multiplicative decrease; logs `aimd_decrease` at info or warning depending on whether the floor was reached.

`AsyncAdaptiveSemaphore._handle_success(slot_epoch)` — synchronous; three guard checks: stale epoch, grace period, increase interval throttle. Additive increase only after all guards pass.

`AsanaResponseHandler._parse_rate_limit_error(response)` — extracts `Retry-After` header with `contextlib.suppress(ValueError)` to handle malformed header values gracefully. Returns `RateLimitError(retry_after=None)` if header is absent or malformed.

**Configuration surface** (in `src/autom8_asana/config.py`):
- `RateLimitConfig`: `max_requests` (from settings, default 1500/60s), `window_seconds=60`
- `ConcurrencyConfig`: `read_limit=50`, `write_limit=15`, `aimd_enabled=True`, `aimd_floor=1`, `aimd_multiplicative_decrease=0.5`, `aimd_additive_increase=1.0`, `aimd_grace_period_seconds=5.0`, `aimd_increase_interval_seconds=2.0`, `aimd_cooldown_trigger=5`, `aimd_cooldown_duration_seconds=30.0`
- `RetryConfig`: `max_retries=5`, `base_delay=0.5`, `max_delay=60.0`, `exponential_base=2.0`, `jitter=True`, `retryable_status_codes={429, 503, 504}`
- `CircuitBreakerConfig`: `enabled=False` by default (opt-in, TRADE-002), `failure_threshold=5`, `recovery_timeout=60.0`, `half_open_max_calls=1`
- `TimeoutConfig`: `connect=5.0`, `read=30.0` — only `connect` is passed to the platform SDK (IMP-10); `read` is not currently translated
- `ConnectionPoolConfig`: `max_connections=100`, `max_keepalive_connections=20`

**Internal imports (transport package only):**
- `autom8_asana.errors` — `AsanaError`, `RateLimitError`, `ServerError`, `TimeoutError`, `SyncInAsyncContextError`, `ConfigurationError` (note: module renamed `exceptions.py` → `errors.py` in PKG-007/`a1fc3955`)
- `autom8_asana.config` — config dataclasses, via `TYPE_CHECKING` only in most files
- `autom8_asana.protocols.auth` — `AuthProvider` (TYPE_CHECKING only)
- No direct `httpx` import (TID251 lint ban enforced for this package)

**Test coverage** (`tests/unit/transport/`): 7 dedicated test files — `test_adaptive_semaphore.py`, `test_aimd_integration.py`, `test_aimd_simulation.py`, `test_asana_http.py`, `test_config_translator.py`, `test_response_handler.py`, `test_sync_wrapper.py`. This is unusually thorough test coverage for the transport layer relative to other packages.
...
| `TimeoutException` | `circuit_breaker.record_failure()`, retry up to `max_retries` | `TimeoutError` raised after exhaustion |
...
| 5 consecutive 429s | `aimd_cooldown_threshold_reached` warning logged | No automatic cooldown in v1 — FR-008 stub only (TRADE-003) |
...
| Multipart upload header mutation | `finally` block restores `content-type` | No leak of modified header state |
| Fuzzed/non-ASCII token | ASCII sanitize with `errors="replace"` in `_get_client` | No `UnicodeEncodeError`; non-ASCII chars replaced with `?` |

**Known architectural friction points:**

1. **`platform_client._client` private access** — `asana_http.py` calls `platform_client._client.request(...)`, bypassing the platform client's public API. This is the cost of managing policies externally. If `autom8y-http` renames or restructures `_client`, the transport layer breaks silently at runtime.

2. **`_request` / `_request_paginated` structural duplication** — The two inner loops are nearly identical (same retry/AIMD/circuit-breaker logic). A bug fix in one must be applied to both.

3. **`CircuitBreakerConfig.enabled=False` default (TRADE-002)** — The circuit breaker is disabled by default for backward compatibility. The `_request` loop calls `self._circuit_breaker.check()` and `record_failure()` unconditionally, but the `CircuitBreaker` implementation from `autom8y-http` must handle `enabled=False` as a no-op. If the platform SDK changes this behavior, all circuit-breaker calls in the transport become no-ops silently.

4. **`cooldown_duration_seconds` is unused in v1 (TRADE-003)** — `AIMDConfig` documents this as a placeholder. The cooldown warning fires at 5 consecutive 429s but does not reduce throughput further. Operators seeing the `aimd_cooldown_threshold_reached` warning have no further automatic protection beyond the AIMD window halving.

5. **`stream` and `get_stream_url` skip AIMD** — Both streaming methods call `rate_limiter.acquire()` but do not acquire the concurrency semaphore. Long-running streaming operations do not consume a concurrency slot, so `in_flight` undercounts actual API connections during streaming.
...
7. **`TimeoutConfig.read` not translated** — `ConfigTranslator.to_http_client_config` only passes `timeout.connect` to `HttpClientConfig.timeout` (documented as IMP-10 for faster failure detection). The `read`, `write`, and pool timeout values are not passed. If `HttpClientConfig.timeout` is a scalar, the read timeout effectively equals the connect timeout.
...
2. **`ExponentialBackoffRetry.max_attempts` vs. `RetryConfig.max_retries`** — `_should_retry` uses `self._retry_policy.max_attempts`. Whether this equals `max_retries` or `max_retries + 1` (initial attempt counted) is not verifiable from the source here.
...
4. **`get_stream_url` return type annotation** — The method is annotated `-> AsyncIterator[bytes]` but the function body is an async generator (`yield` inside `async def`). It is NOT decorated with `@asynccontextmanager` like `stream()`, so callers cannot use it as an async context manager. The relationship between `stream()` and `get_stream_url()` is not documented in either method.

5. **Thread-safety of `AsyncAdaptiveSemaphore._handle_reject` / `_handle_success`** — Both methods are documented as "synchronous — no await points." This is safe for asyncio's single-threaded concurrency model, but if `AsanaHttpClient` is ever used from multiple threads (e.g., via `asyncio.run()` in a thread pool), the semaphore state could be corrupted. The scar-tissue doc (SCAR-010/010b) shows prior threading issues in the persistence layer; the transport has no equivalent lock.
...
source_files_read:
  ...
  - src/autom8_asana/transport/sync.py
  - src/autom8_asana/config.py (transport-relevant sections)
  - src/autom8_asana/errors.py (transport imports)
  - .know/architecture.md
  - .know/design-constraints.md (TRADE-002, TRADE-003)
commits_since_prior_hash:
  - 9bb06bd8: PKG-014/015/016 convention sweep
  - a1fc3955: PKG-007 rename exceptions.py -> errors.py
  - 6b93faf4: schemathesis fuzz hardening (ASCII token encoding added)
  - a114c619: IMP-10 connect timeout for faster failure detection
delta_from_prior:
  - Import path updated: autom8_asana.errors (was autom8_asana.exceptions)
  - ASCII token sanitization in _get_client documented (new defensive pattern)
  - IMP-10 connect-only timeout rationale explicitly documented
  - ADR-0048 circuit breaker reference added
  - CircuitBreakerConfig.half_open_max_calls added to config surface
  - ConnectionPoolConfig.max_keepalive_connections added to config surface
  - Removed line-number references (shifted post-edits)
  - Knowledge gap 7 (read timeout not translated) added
```
