...
**Status**: PARTIAL (in-repo COMPLETE) — request span + 3 structured logs + 4 Prometheus metrics landed 2026-06-01 (branch `sre-ob2-exports-observability-2026-06-01`). Residual is cross-repo only: SLO targets + burn-rate alert rules are Terraform in the autom8y MONOREPO (asana has 0 `.tf`) → handoff `HANDOFF-sre-to-monorepo-exports-slo-alerts-2026-06-01.md`. | **Severity**: P2 | **Pre-GA Deadline**: 2026-06-15
...
**Reliability gap — receiver bulk fan-out concurrency starvation (RECV-BULK-001)**: Under a 104-project bulk `refresh_project_frames` fan-out, the receiver collapsed below 82% satellite-success and tripped ECS task replacement. Contributing surface: `BuildCoordinator.max_concurrent_builds = 4` (`cache/dataframe/build_coordinator.py:130-141`) plus single uvicorn worker. The width axis is project COUNT, not section count — `section_gid`/`section_index` are grep-zero in `services/universal_strategy.py` (0/1277 lines) and section is post-build column classification (lines 447-513), so the build key `(project_gid, entity_type)` (build_coordinator.py:51,54; universal_strategy.py:922) is NOT multiplied by sections. Verdict: `.ledge/decisions/SRE-VERDICT-receiver-bulk-validation-2026-06-02.md`. Open at 2026-06-02.

**Reliability gap — task restart orphans in-flight builds (RECV-BULK-002)**: During bulk load, 2 builds started for one gid (09:17:39-40) with ZERO `build_coordinator_completed`; a new `build_coordinator_initialized` at 09:20:18 = mid-build task replacement, corroborated by ELB-502s 09:19-09:20. Builds are killed, not drained — converting degradation into hard 5xx for in-flight callers. Recovery is auto-replacement (unhealthy→initial→healthy ~90s), NOT graceful in-place degradation. Fix direction: SIGTERM drain hold + ECS health-check threshold tuning. Open at 2026-06-02.

**Reliability gap — singleflight dedup UNPROVEN under live load (RECV-BULK-003)**: The bulk load only ever generated 2 distinct-key builds for one gid, both orphaned before any `build_coordinator_coalesced` event (coalesced path at build_coordinator.py:194). Singleflight coalescing correctness has no live receipt; the 503 `Retry-After` HTTP header is structurally wired (`api/errors.py:621-627`, conditional on `exc.details["retry_after_seconds"]`) but live-firing-under-bulk is unproven. Requires a chaos experiment asserting concurrent same-key requests produce `coalesced` count > 0 + one `completed`. Open at 2026-06-02.
...
| LOG-TRACE-LAMBDA | Lambda handlers: `add_otel_trace_ids` + custom processors | P3 | None declared | RESOLVED 2026-06-02 — premise corrected: `add_otel_trace_ids` is ALREADY in the autom8y_log SDK default base chain (`structlog_backend.py:88`), so trace-id↔log correlation worked all along (incl. Lambda). The REAL defect surfaced here was that `core.logging.configure(additional_processors=...)` silently dropped custom processors (dead `_filter_sensitive_data`) due to SDK auto-config ordering — root-fixed PR #89 (`reset_logging()` before configure). See `.ledge/decisions/SRE-VERDICT-log-trace-lambda-2026-06-02.md` + memory `logging-custom-processors-dead-by-sdk-autoconfig`. |
