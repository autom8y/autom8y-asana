...
class BusinessSeeder:
    ...
    async def _find_business_async(self, data: BusinessData) -> Business | None:
        ...
        try:
            ...
            if match_result:
                return match_result
        except Exception as e:  # BROAD-CATCH: catch-all-and-degrade -- composite matching involves search+blocking+scoring, must degrade gracefully  # noqa: BLE001
            logger.warning(
    ...
    async def _search_by_company_id(self, company_id: str) -> SearchHit | None:
        ...
        except Exception as e:  # BROAD-CATCH: catch-all-and-degrade -- search API + assertion can raise diverse errors  # noqa: BLE001
            logger.warning(
